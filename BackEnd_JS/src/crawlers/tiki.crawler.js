...
export class TikiCrawler {
  ...
  async crawl(productName) {
    try {
      ...
      // 2. Navigate to Tiki search page
      ...
      logger.info(`[Tiki] Found ${products.length} products`);
      return products;
    } catch (error) {
      logger.error('[Tiki] Crawl error:', error);
      return [];
    }
  }
  ...
  async extractProducts(page) {
    ...
    try {
      ...
      logger.error('[Tiki] Extract products error:', error);
      return [];
    }
  }
}
